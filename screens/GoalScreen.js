...
const CircularProgress = ({ percentage = 50, radius = 40, strokeWidth = 8, color = '#2E4E45' }) => {
...
export default function GoalsScreen() {
  ...
  const [objectiveDropdownVisible, setObjectiveDropdownVisible] = useState(false);
  ...
  const [generalGoals, setGeneralGoals] = useState([
    { id: 1, label: 'Riduci le sigarette settimanali sotto 50', completed: false },
    { id: 2, label: 'Risparmia 100€ al mese', completed: false },
    { id: 3, label: 'Fai sport almeno 3 volte a settimana', completed: true },
  ]);

  const currentGoals = objectiveViewMode === 'giornalieri' ? dailyGoals : generalGoals;

  const toggleGoal = (id) => {
    const updateGoals = (goals) => goals.map(goal => goal.id === id ? { ...goal, completed: !goal.completed } : goal);
    objectiveViewMode === 'giornalieri' ? setDailyGoals(updateGoals(dailyGoals)) : setGeneralGoals(updateGoals(generalGoals));
  };
    ...
    <DeviceFrame>
      <View style={styles.wrapper}>
        <ScrollView contentContainerStyle={styles.container}>
          <Text style={styles.title}>Ciao ****, ecco gli obiettivi da portare a termine!</Text>
          ...
          <View style={styles.progressContainer}>
            ...
            <View style={{ alignItems: 'center' }}>
              ...
              <Text style={styles.progressLabel}>OBIETTIVI {objectiveViewMode.toUpperCase()}</Text>
            </View>
          </View>

          <TouchableOpacity style={styles.setGoalsButton} onPress={() => setNewGoalModalVisible(true)}>
          ...
          <View style={styles.goalCard}>
            <View style={styles.cardHeader}>
              ...
              <TouchableOpacity
                ...
                onPress={() => setObjectiveDropdownVisible(!objectiveDropdownVisible)}
              >
            ...
            {currentGoals.map((goal) => (
              <TouchableOpacity key={goal.id} style={styles.goalItem} onPress={() => toggleGoal(goal.id)}>
                <Text style={styles.goalText}>{goal.label}</Text>
                <Ionicons name={goal.completed ? 'checkbox-outline' : 'square-outline'} size={22} color="#2E4E45" />
              </TouchableOpacity>
            ))}
            ...
            {objectiveDropdownVisible && (
              <View style={styles.overlayDropdown}>
                {['giornalieri', 'generali'].map((option) => (
                  <TouchableOpacity key={option} style={styles.dropdownItem} onPress={() => {
            ...
            <View style={styles.goalModalBox}>
              <View style={styles.goalModalHeader}>
                <Text style={styles.goalModalTitle}>
                  Riempi il form con il tuo prossimo obiettivo o generalo in automatico
                </Text>
              ...
              <View style={styles.goalModalControls}>
                ...
                <TouchableOpacity
                  onPress={() => {
                    const auto = newGoalType === 'giornaliero'
              ...
              <View style={styles.goalModalButtons}>
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    const newGoal = { id: Date.now(), label: newGoalText, completed: false };
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 26, fontWeight: '700', color: '#2E4E45', textAlign: 'center', marginBottom: 20 },
  progressContainer: { flexDirection: 'row', justifyContent: 'space-around', marginBottom: 30 },
  ...
  setGoalsButton: {
    flexDirection: 'row', backgroundColor: '#2E4E45', paddingVertical: 12, paddingHorizontal: 24,
    borderRadius: 24, alignItems: 'center', alignSelf: 'center', marginBottom: 24,
  },
  exportButton: {
    flexDirection: 'row', backgroundColor: '#2E4E45', paddingVertical: 14, paddingHorizontal: 28,
    borderRadius: 24, alignItems: 'center', justifyContent: 'center', alignSelf: 'center', marginTop: 30,
  },
  ...
  dropdownToggle: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#EAEAEA',
    borderRadius: 10, paddingHorizontal: 10, paddingVertical: 4,
  },
  dropdownText: { fontSize: 14, marginRight: 4, color: '#333' },
  overlayDropdown: {
    position: 'absolute', top: 48, right: 10, backgroundColor: '#fff', borderRadius: 10,
    padding: 10, elevation: 10, zIndex: 1000, borderWidth: 1, borderColor: '#ccc',
  },
  ...
  goalModalBox: { width: 300, backgroundColor: '#fff', borderRadius: 16, padding: 16, alignItems: 'center' },
  goalModalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, width: '100%' },
  goalModalTitle: { flex: 1, fontSize: 14, fontWeight: 'bold', color: '#333' },
  goalModalControls: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', marginVertical: 10 },
  goalTypeButton: { paddingHorizontal: 10, paddingVertical: 6, backgroundColor: '#eee', borderRadius: 6 },
  goalTypeSelected: { backgroundColor: '#cde3da' },
  goalModalTextarea: { width: '100%', height: 100, backgroundColor: '#f7f7f7', borderRadius: 8, marginVertical: 10 },
  goalInput: { padding: 10, height: '100%', textAlignVertical: 'top' },
  goalModalButtons: { flexDirection: 'row', justifyContent: 'space-between', width: '100%' },
  confirmBtn: { flex: 1, marginHorizontal: 6, borderRadius: 8, paddingVertical: 12, alignItems: 'center' },
  confirmText: { color: '#fff', fontWeight: 'bold' },
  navbar: {
    position: 'absolute', bottom: 0, width: '100%', height: 70,
    flexDirection: 'row', justifyContent: 'space-around', alignItems: 'center',
    borderTopColor: '#ccc', borderTopWidth: 1, backgroundColor: '#fff',
  },
});